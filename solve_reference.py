import pulp

nodes = [0,3,10,21,22,35,40,41,47,65,71,76,77]
n = len(nodes)
idx = {node:i for i,node in enumerate(nodes)}

# Distance matrix exactly as in the provided table (rows/cols in node order above)
D = [
 [0,29,18,16,24,32,17,28,29,27,27,33,34],
 [29,0,38,40,29,48,15,57,27,53,55,24,44],
 [18,38,0,31,42,49,22,27,21,19,36,50,16],
 [16,40,31,0,21,18,32,25,45,30,16,34,47],
 [24,29,42,21,0,20,30,46,46,48,36,14,56],
 [32,48,49,18,20,0,45,40,60,47,25,33,65],
 [17,15,22,32,30,45,0,44,16,39,45,32,30],
 [28,57,27,25,46,40,44,0,48,11,17,59,41],
 [29,27,21,45,46,60,16,48,0,40,54,48,18],
 [27,53,19,30,48,47,39,11,40,0,26,60,30],
 [27,55,36,16,36,25,45,17,54,26,0,50,52],
 [33,24,50,34,14,33,32,59,48,60,50,0,62],
 [34,44,16,47,56,65,30,41,18,30,52,62,0],
]

# sanity check symmetry
for i in range(n):
    for j in range(n):
        assert D[i][j] == D[j][i], (i,j)
    assert D[i][i] == 0

prob = pulp.LpProblem("TSP_MTZ", pulp.LpMinimize)

x = pulp.LpVariable.dicts("x", (range(n), range(n)), cat="Binary")
u = pulp.LpVariable.dicts("u", range(n), lowBound=1, upBound=n-1, cat="Continuous")

# objective
prob += pulp.lpSum(D[i][j]*x[i][j] for i in range(n) for j in range(n) if i!=j)

# no self loops
for i in range(n):
    prob += x[i][i] == 0

# single exit
for i in range(n):
    prob += pulp.lpSum(x[i][j] for j in range(n) if j!=i) == 1

# single entry
for j in range(n):
    prob += pulp.lpSum(x[i][j] for i in range(n) if i!=j) == 1

# MTZ subtour elimination (depot = index 0 excluded from u)
for i in range(1, n):
    for j in range(1, n):
        if i != j:
            prob += u[i] - u[j] + (n-1)*x[i][j] <= n-2

solver = pulp.PULP_CBC_CMD(msg=False)
prob.solve(solver)

print("Status:", pulp.LpStatus[prob.status])
print("Objective (km):", pulp.value(prob.objective))

# extract arcs
arcs = []
for i in range(n):
    for j in range(n):
        if i!=j and pulp.value(x[i][j]) > 0.5:
            arcs.append((nodes[i], nodes[j], D[i][j]))

print("\nArcos seleccionados:")
for a in arcs:
    print(a)

# reconstruct sequence starting at depot (node 0)
seq = [0]
current = 0
arc_map = {a[0]:a[1] for a in arcs}
while True:
    nxt = arc_map[current]
    seq.append(nxt)
    current = nxt
    if current == 0:
        break

print("\nSecuencia de visita:")
print(" -> ".join(str(s) for s in seq))
print("\nNumero de arcos:", len(arcs))
print("Verificacion distancia total sumando arcos:", sum(a[2] for a in arcs))
